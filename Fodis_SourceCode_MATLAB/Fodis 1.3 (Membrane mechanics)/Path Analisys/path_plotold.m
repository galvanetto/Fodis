function [TracesGroup]=path_plot(gp,sizeMarker)
...
for ii=1:size(ia,1)
    ...
    plot(iaa-1,c,'LineWidth',sizeMarker*ripetition(ii)/2);
...
for ii=1:size(gp1,1)
    for jj=1:ii
        if(punti(jj,ii)>0)
            plot(ii,jj,'.','MarkerSize',7*(punti(jj,ii)^(2/3)),'Color',RGB(ii,:));
        end
    end
end

%Plot first Dot
plot(0,0,'.','MarkerSize',(7*size(gp1,2)^(2/3)),'Color','k');
