function [FcMax, FcProfile, xBinLcHistMax, LcHistVar, LcMax] = getMaxLc2(Lc, Fc, xBinMax, xBin, zerosTempMax, thresholdNPoints)

%NEW FUNCTION FOR FINDING MAXIMA (replace getMaxLc())
...
%new assignation of LcMax and FcMax


LcHist = hist(Lc, xBinMax);
FcMax = zeros(1, length(xBinMax));

LcHist(LcHist <= thresholdNPoints) = 0;  %subtract Npoints
[value, idxposition]=findpeaks(LcHist);

LcMax=xBinMax(idxposition);

FcMax(idxposition)=FcProfile(idxposition)+1e-12;
