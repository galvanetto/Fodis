function varargout = fingerprint_ROI_2(varargin)
%Gui function to manually select regions in F-tss space where point of the
%races must fall into
%it uses brush command

% FINGERPRINT_ROI_2 MATLAB code for fingerprint_ROI_2.fig
%      FINGERPRINT_ROI_2, by itself, creates a new FINGERPRINT_ROI_2 or raises the existing
%      singleton*.
%
%      H = FINGERPRINT_ROI_2 returns the handle to a new FINGERPRINT_ROI_2 or the handle to
%      the existing singleton*.
%
%      FINGERPRINT_ROI_2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FINGERPRINT_ROI_2.M with the given input arguments.
%
%      FINGERPRINT_ROI_2('Property','Value',...) creates a new FINGERPRINT_ROI_2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before fingerprint_ROI_2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to fingerprint_ROI_2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help fingerprint_ROI_2

% Last Modified by GUIDE v2.5 22-May-2019 19:33:48

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @fingerprint_ROI_2_OpeningFcn, ...
                   'gui_OutputFcn',  @fingerprint_ROI_2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function fingerprint_ROI_2_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = fingerprint_ROI_2_OutputFcn(hObject, eventdata, handles) 
...
xlim([0 abs(data.scaleMaxTss)]);   xlabel('tss (nm)');
ylim([0 abs(data.scaleMaxF)]); ylabel('F (pN)');

if data.scaleMaxTss<0 || data.scaleMaxF<0
   beep
    Mess = msgbox({['Your trace is not in the correct position .'];['(peaks in the positive part, contact point in the left)'...
       ];['--------------------'];['Please go to Tools>Absolute height to TSS  and try to adjust the trace.']...
       ;['There may be a problem with data dimensions, baseline subtraction or TSS transformation.']});
end
