...
intexstr={'.txt','.jpk-force','.txt','.spm','.txt','.txt'};   %add here if you add a mode of import
...
%% preallocate traces
% don't preallocate for indexpop==6
if indexpop~=6
    ...
    temp.tracesExtend = cell(1, 2);
    temp.tracesRetract = cell(1, 2);
    data.position =zeros(1, 2);
    temp.translateLc= zeros(1,1);
end
%%
...
for ii = 1:1:nFiles
    try
        ...
        elseif indexpop==6
            [temp.tracesExtend(1,:),temp.tracesRetract(1,:)] = readTxt_forget(files{ii});
            
            file_rows=fopen('a_rows.txt','a');
            fprintf(file_rows,'%g ',-temp.tracesRetract{1,2});  %force first with minus sign
            fprintf(file_rows,'\n');
            fprintf(file_rows,'%g ',temp.tracesRetract{1,1});
            fprintf(file_rows,'\n');
            fclose(file_rows);
